			 		...
			 		if( $(this).hasClass('menu_submenu')   ) {
				 		...
				 		if( $(this).val()  && $(this).val() == 1 && !$(item).hasClass( 'dropdown-toggle' ) ) {
				 			...
				 			$(item).attr( 'data-toggle', 'leo-dropdown' );
				 		 	...
				 		 	var div = '<div class="dropdown-sub dropdown-menu" style="left:100%"><div class="dropdown-menu-inner"><div class="row active"></div></div></div>';// left http://screencast.com/t/JURUru3FVbK9
		...
		function listenEvents( $megamenu ){
			...
			$( 'a', $megamenu ).click( function(event){
				...
				$(".mega-col", $megamenu).removeClass('active');
				 	var pos =  $(this).offset();
				    $('#menu-form').css('left',pos.left  + $("#verticalmenu").width() );	// left http://screencast.com/t/5cIf8gNTdO2
					$('#menu-form').css('top',pos.top - $('#menu-form').height() );
					
				$( ".dropdown-menu",$parent.parent()).hide();
	 	...
	 	function removeRowActive(){
	 		$('#column-form').hide();
 			$( "#leo-verticalmenu .row.active" ).removeClass('active');
	 	}
	 	...
	 	function removeColumnActive(){
	 		...
	 		$( "#leo-verticalmenu .mega-col.active" ).removeClass('active');
	 	}
	 	...
	 	function removeMenuActive(){
	 		$('.form-setting').hide();
	 		$( "#leo-verticalmenu .open" ).removeClass('open');
	 		$( "#leo-verticalmenu .row.active" ).removeClass('active');
 			$( "#leo-verticalmenu .mega-col.active" ).removeClass('active');
 			if( activeMenu ) {	
		 		activeMenu = null;
	 	...
	 	function saveMenuData(){
	 	 	var output = new Array();	
	 	 	 $("#megamenu-content #leo-verticalmenu li.parent").each( function() {
				 	var data = $(this).data();
				 	data.rows = new Array();
	 	...
	 	function loadWidgets(){
	 		...
	 		$("#megamenu-content #leo-verticalmenu .mega-col").each( function() {
	 		 	var col = $(this);		
	 		...
	 		$.each( ajaxCols, function (i, col) {
	 			$.ajax({
					...
					type:'POST',
			 		...
			 		col.children('div').html( data );
			 		cnt++;  
			 		$("#leosb-progress .progress-bar").css("width", (cnt*100)/ajaxCols.length+"%" );
			 		if( ajaxCols.length == cnt ){
			 			$("#megamenu-content").delay(1000).fadeIn();
			 			$("#leosb-progress").delay(1000).fadeOut();
			 		}
	 	...
	 	function reloadMegamenu(){
			...
			var megamenu = $("#megamenu-content #verticalmenu");
