    ...
    public function processAJax()
    {
        ...
        if (Tools::getValue('action') && Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
            ...
            foreach ($slides as $position => $id_slide) {
                $sql = 'UPDATE `'._DB_PREFIX_.'leosliderlayer_slides` SET `position` = '.(int)$position.'
...
//					foreach ($layer as $k => &$l)
    ...
    public function getSliderGroupByHook($hookName = '', $active = 1)
    {
        $this->context = Context::getContext();
        $id_shop = $this->context->shop->id;
        $sql = 'SELECT * FROM '._DB_PREFIX_.'leosliderlayer_groups gr
                WHERE gr.id_shop = '.(int)$id_shop.'
                AND gr.hook = "'.$hookName.'"'.($active ? ' AND gr.`active` = 1' : ' ').'
    ...
    public function getSlides($id_group, $active = null)
    {
        ...
        $sql = 'SELECT lsl.`id_leosliderlayer_slides` as id_slide, lsl.*,lsll.*
        ...
        if (!$this->isCached($tpl, $this->getCacheId($hookName.'_'.$this->name))) {
            ...
            foreach ($sliders as $key => $slider) {
                ...
                if ($slider['video']['usevideo'] == 'youtube' || $slider['video']['usevideo'] == 'vimeo') {
                    $slider['videoURL'] = Tools::getCurrentUrlProtocolPrefix() . 'player.vimeo.com/video/'.$slider['video']['videoid'].'/';
                    if ($slider['video']['usevideo'] == 'youtube') {
                        $slider['videoURL'] = Tools::getCurrentUrlProtocolPrefix() . 'www.youtube.com/embed/'.$slider['video']['videoid'].'/';
                    }
                }
